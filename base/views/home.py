...
def home(request):
    ...
    topics_only = request.GET.get("topic") if request.GET.get("q") else ""
    page = int(request.GET.get("page")) if request.GET.get("page") else 1

    order_by = "-updated" if request.GET.get("filter") == "recent" else "participants"
    ...
    if topics_only:
        rooms = rooms.filter(Q(topic__name__icontains=q))
    ...
    if request.user.is_authenticated:
        rooms = rooms.filter(Q(private=False) | Q(participants=int(request.user.id)))
    else:
